...
class CSearch
{
	...
	function Search(&$phrase, $page, $next)
    {
		...
		if($this->datacount > 0)
        {
            ...
            {
				if((filemtime($this->datapath . "/index.lck") + 300) < time())
                    unlink($this->datapath . "/index.lck");
				usleep(500000);
				$i++;
				if($i > 9)
                    die("Index.dat is locked! Please try it later again!");
			}
            ...
            {
				if((filemtime($this->datapath . "/data.lck") + 300) < time())
                    unlink($this->datapath . "/data.lck");
				usleep(500000);
				$i++;
				if($i > 9)
                    die("Data.dat is locked! Please try it later again!");
			}
			...
			while(($position >= 0 && $entrynumber > 0) || ($page >= 1 && $entrynumber == 0 && $this->datacount > 0))
            {
				...
				if($entrynumber > $this->entriesperpage)
                    $entrynumber = $this->entriesperpage;
				...
				if(count($result) > 0)
                {
					...
					if(isset($lang))
                        header("Location: guestbook.php?act=show&lang=" . $lang . "&phrase=" . $phrase . "&page=" . $page . "&entrypos=" . implode(":", $result) . "#searchresult");
					else
                        header("Location: guestbook.php?act=show&phrase=" . $phrase . "&page=" . $page . "&entrypos=" . implode(":", $result) . "#searchresult");
					exit();
				}
